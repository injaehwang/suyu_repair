        ...
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-12">
            ...
            <p className="text-slate-500">
              가죽자켓, 코트, 패딩부터 청바지, 정장까지 모든 의류 수선
              가능합니다.
            </p>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
            <CategoryCard
              title="가죽/모피"
              services={["가죽자켓", "무스탕", "가죽염색"]}
            />
            <CategoryCard
              title="패딩/아웃도어"
              services={["패딩수선", "충전재보강", "지퍼교체"]}
            />
            <CategoryCard
              title="코트/자켓"
              services={["코트기장", "소매수선", "안감교체"]}
            />
            <CategoryCard
              title="바지/데님"
              services={["청바지기장", "허리수선", "통줄임"]}
            />
            <CategoryCard
              title="정장/셔츠"
              services={["정장리폼", "셔츠카라", "소매기장"]}
            />
            <CategoryCard
              title="명품/리폼"
              services={["명품수선", "전체리폼", "디자인변경"]}
            />
          </div>
        </div>
      </section>
        ...
        <div className="container mx-auto max-w-6xl">
          <div className="text-center mb-12">
            <h2 className="text-2xl md:text-4xl font-bold text-slate-900 mb-4 tracking-tight leading-snug">
              세 단계로 끝나는 간편한 수선, <br className="md:hidden" /> 함께
              보실까요?
            </h2>
          </div>
...
function StepCard({
  ...
  return (
    <div className="bg-white rounded-3xl p-6 md:p-8 shadow-sm hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1 border border-slate-100 flex flex-row md:flex-col items-center md:items-center text-left md:text-center group gap-4 md:gap-0">
      ...
      <div>
        <span className="text-xs font-bold text-slate-400 tracking-wider mb-1 md:mb-2 block">
          STEP {step}
        </span>
    ...
    <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-100 hover:shadow-md transition-all text-center">
      ...
      <ul className="text-sm text-slate-500 space-y-1">
        {services.map((service, idx) => (
          <li
            key={idx}
            className="bg-slate-50 rounded-md py-0.5 px-2 inline-block m-0.5"
          >
            {service}
          </li>
        ))}
