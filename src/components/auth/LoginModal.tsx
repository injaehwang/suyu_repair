                    ...
                    {mode === 'login' ? (
                        <>
                            {process.env.NODE_ENV === 'development' ? (
                                /* Dev Login (개발 모드) - 이메일만으로 로그인 */
                                <form onSubmit={handleCredentialsLogin} className="space-y-3 mb-4">
                                    <div className="p-3 bg-amber-50 border border-amber-200 rounded-xl">
                                        <p className="text-xs font-bold text-amber-700">개발 모드 - 이메일만 입력하면 로그인됩니다</p>
                                    </div>
                                    <input
                                        type="email"
                                        value={email}
                                    ...
                                    <Button
                                        type="submit"
                                        className="w-full h-12 rounded-full bg-amber-600 hover:bg-amber-700 text-white font-bold text-sm relative"
                                        disabled={!!isLoading}
                                    >
                                        {isLoading === 'credentials' ? '로그인 중...' : '개발 로그인'}
                                        {isLoading === 'credentials' && (
                                            <div className="absolute inset-0 bg-amber-600/80 flex items-center justify-center rounded-full">
